...
function main() {
  ...
  canvas.addEventListener("click", async function(event) {
    if (document.pointerLockElement === canvas) {
      //console.log("Click!");
      let pos = rayTrace(camera);
      if (pos[0] != -1) {
        if (event.shiftKey) {
          camera.currentChunk.mine(pos[0],pos[1],pos[2]);
          animation_running[3] = true;
        }

        else {
          camera.currentChunk.place(pos[0],pos[1],pos[2], camera, 1)
        }
...
function generateStartingChunks() {
  ...
  posToChunkMap.set(tunrPosToString(0,0), mainChunk);
...
function placeEndermen(n) {
  for (i = 0; i < n; i++) {
    let randX = getRandomInt(16);
    let randZ = getRandomInt(16);
    let randChunk = chunksToRender[getRandomInt(chunksToRender.length)];
    let randY = randChunk.findHighestBlock(randX, randZ)+2;
    var e = new Enderman([randX + (16 * randChunk.x),randY, randZ + (16 * randChunk.y)]);
    Endermen.push(e);
  }
}
function renderScene() {
  ...
  //var globalRotMat = new Matrix4().rotate(g_globalAngle[0]*-1,1,0,0); //activate global rotation matrix
  //globalRotMat = globalRotMat.rotate(g_globalAngle[1]*-1,0,1,0);
  //globalRotMat = globalRotMat.rotate(g_globalAngle[2]*-1,0,0,1);
  ...
  sky.transformations.push([Transform.Translate,[-100,-100,-100]]);
  sky.transformations.push([Transform.Scale,[1000,1000,1000]]);
  sky.render(origin);
  //TEST
  /*var base = new Cube();
  ...
  leftArm.render();*/
  //let vertices = mergeArrays([vertexBufferCube[0].elements, vertexBufferCube[1].elements, vertexBufferCube[2].elements]);
  //console.log("Debug vert:", vertices);
  //drawTriangle3DUV(vertices, [0,0, 1,0, 0,1]);
}
...
function rayTrace(camera) {
  if (camera.currentChunk != null) {
    ...
    for (var i = 1; i < 10; i++) {
      ...
      let x = Math.floor(tEye.elements[0] % 16);
      let y = Math.floor(tEye.elements[1] % 16);
      let z = Math.floor(tEye.elements[2] % 16);
  
      if (Chunk.withinBounds(x,y,z)) {
        //console.log(x,y,z); 
        curChunk = camera.currentChunk.chunk;
        console.log(curChunk);
        if (curChunk[x][z][y].is_Visible) {
          return [x,z,y];
        }
      }
    }
